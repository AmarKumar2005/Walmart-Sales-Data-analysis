...
GST FLOAT(6,4) NOT NULL,   -- In place of VAT we change into Gst.(challange)
...
UPDATE sales AS a
JOIN (SELECT AVG(total) AS avg_total FROM sales) AS b
SET a.product_category = CASE
...
-- 3.Which city has the largest tax percent/ GST (GOODS SERVICES TAX)?
SELECT city, SUM(GST) AS total_GST
FROM sales GROUP BY city ORDER BY total_GST DESC LIMIT 1;

-- 4.Which customer type pays the most in GST?
SELECT customer_type, SUM(GST) AS total_GST
FROM sales GROUP BY customer_type ORDER BY total_GST DESC LIMIT 1;
...
from sales where branch = 'A' group by time_of_day,branch order by rating desc;
...
From sales where branch = 'A' Group by day_name, branch order by avg_rating desc LIMIT 1;
